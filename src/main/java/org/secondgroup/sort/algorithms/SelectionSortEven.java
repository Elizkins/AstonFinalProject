...
public class SelectionSortEven {
    ...
    private static <T extends Student> int defineField(Comparator<T> compar) {
        if (compar instanceof GroupNumberComparator) {
            System.out.println("поля \"Номер группы\"");
            return 1;
        } else if (compar instanceof AvgGradeComparator) {
            System.out.println("поля \"Средний балл\"");
            return 2;
        } else if (compar instanceof RegBookComparator) {
            System.out.println("поля \"Номер зачетной книжки\"");
            return 3;
        } else {
